...
public:
    AvgCov(unsigned int size, std::string name, bool avgOnly = false, unsigned int showPeriod = 1000, unsigned int waitCount = 5000, double zoomFactor = 10000)
        : _exp(), _cov(), _defaultWeight(), _avgOnly(avgOnly), _size(size), _measureCount(0), _showPeriod(showPeriod), _waitCount(waitCount), _zoomFactor(zoomFactor), _valueName(name) {
        _exp.resize(size);
